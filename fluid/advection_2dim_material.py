...
"""
風上差分で2次元移流方程式を解く/ベクトルではなくスカラー(物質の濃度とか)の輸送
"""
...
def background():
    ...
    for i in range(nX):
        for j in range(nY):
            cx[i+1][j+1] =   omega * 10**2 * (j*dy-nY*dy/2) / r(i,j)**2
            cy[i+1][j+1] = - omega * 10**2 * (i*dx-nX*dx/2) / r(i,j)**2
...
def initCondition() :
    ...
    # def init_rho2() : #棒
    #     if (j<3 and j>-3 and i>

    rho = np.zeros([nX+2, nY+2])

    for i in range(nX):
        for j in range(nY):
            rho[i+1][j+1] = init_rho(j,i)
